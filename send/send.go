...
func sendFiles() {
	// Note: Make sure these files actually exist on this device
	files := []string{
		"D:\\university\\fourth\\2nd\\DDB\\Section\\Practice-DDB-main\\Section 5\\Scripts\\Data\\gene.fna",
		"D:\\university\\fourth\\2nd\\DDB\\Section\\Practice-DDB-main\\Section 5\\Scripts\\Data\\gene2.fna",
		"D:\\university\\fourth\\2nd\\DDB\\Section\\Practice-DDB-main\\Section 5\\Scripts\\Data\\fgenome.fa",
	...
	// IMPORTANT: Replace this IP with the actual IP address of the receiving device
	receiverURL := "http://192.168.1.170:9080/save"

	req, err := http.NewRequest("POST", receiverURL, &body)
	if err != nil {
		fmt.Println("Error creating request")
		return
	}
	...
	client := &http.Client{}
	fmt.Println("Sending files to", receiverURL, "...")
	resp, err := client.Do(req)
	if err != nil {
	...
	respBody, _ := io.ReadAll(resp.Body)
	fmt.Println("Response from receiver:", string(respBody))
	fmt.Println("Finished.")
}

func main() {
	// Call the function directly instead of starting a server
	sendFiles()
}
